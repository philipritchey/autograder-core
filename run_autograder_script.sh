...
if [ "${language}" == "c++" ]; then
  ...
  cp $AUTOGRADER_CORE_REPO/support/sql/test_writing.py $TESTBOX/
  cp $AUTOGRADER_CORE_REPO/support/sql/test_compiling.py $TESTBOX/
  cp $AUTOGRADER_CORE_REPO/support/sql/test_running.py $TESTBOX/
